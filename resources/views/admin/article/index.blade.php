        ...
        <div class="card-body">
            <div class="row mb-3">
                ...
                <div class="col-6 text-right">
                    <a href="{{ route('article.create') }}" class="btn btn-success btn-rounded">
                        <i class="mdi mdi-plus"></i>
                        {{ __('Add Article') }}
                    </a>
                </div>
            </div>
            @include('admin.article.partials.filters')
                    ...
                    @forelse ($models as $model)
                        <tr>
                            ...
                            <td class="text-nowrap">
                                <a href="{{ route('article.edit', ['article' => $model->id]) }}" title="{{ __('Edit') }}" class="btn btn-primary btn-sm fa fa-pencil"></a>
                                <a href="{{ route('article.destroy', ['article' => $model->id]) }}" title="{{ __('Delete') }}" class="btn btn-danger btn-sm fa fa-trash" @click="confirmDelete($event)"></a>
                            </td>
                        </tr>
                    @empty
