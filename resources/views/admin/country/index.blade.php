        ...
        <div class="card-body">
            <div class="row mb-3">
                <div class="col-6 mt-2">
                    <h3 class="card-title">{{ __('All Countries') }}</h3>
                </div>
                <div class="col-6 text-right">

                </div>
            </div>
            @include('admin.country.partials.filters')
                ...
                <table class="table">
                    ...
                    @forelse ($models as $model)
                        <tr>
                            ...
                            <td>{{ $model->vat_rate }}</td>
                            <td class="secondary">{{ $model->createdDate }}</td>
                            <td class="secondary">{{ $model->updatedDate }}</td>
                            <td class="text-nowrap">
                                <a href="{{ route('country.edit', ['country' => $model->id]) }}" title="{{ __('Edit') }}" class="btn btn-primary btn-sm fa fa-pencil"></a>
                            </td>
                        </tr>
                    @empty
